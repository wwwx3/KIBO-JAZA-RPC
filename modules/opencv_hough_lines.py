"""
The purpose of this code is to make it easier to crop out the paper(item) from the image in CVAT. Houghlines are mainly used to detect all 
all the straight lines in the image, unlike the Canny edge detector, which outlines all the shapes seen in the picture. 
Script contains functions to process the lines in the  polar coordinate system
returned by the HoughLines function in OpenCV
Line equation from polar to Cartesian coordinates (flatten earth x,y,z coordinates to angles)
x = rho * cos(theta)
y = rho * sin(theta)
...
def hough_lines_intersection(lines: np.array, image_shape: tuple):
    ...
    """
    if lines is None:
        print("No lines to find intersections from.")
        return np.array([], dtype=int)
#Change from == 3 
    if len(lines.shape) == 4 and \
            lines.shape[1] == 1 and lines.shape[2] == 2:
        lines = np.squeeze(lines)
